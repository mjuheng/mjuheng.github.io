...
spring:
  rabbitmq:
    # 开启发送确认（消息发送到交换器后触发回调）
    publisher-confirm-type: correlated
    # 开启发送失败退回（交换机将消息发送到对应队列失败时回调）
...
# 延迟队列
## 原理
![](/images/posts/springboot/rabbitmq-deadletter.png)
...
```java
package org.example.producer.rabbitmq.config;
...
public class DelayRabbitConfig {
    ...
    Queue delayQueuePerMessageTTL() {
        return QueueBuilder.durable("TestDelayQueue")
                .deadLetterExchange("TestDelayReceiveExchange")
                .deadLetterRoutingKey("TestDelayReceiveRouting")
                .ttl(10 * 1000)
                .build();
    }
    ...
    DirectExchange testDelayExchange() {
        return new DirectExchange("TestDelayExchange", true, false);
    }

    @Bean
    Binding bindingDelayDirect() {
        return BindingBuilder.bind(delayQueuePerMessageTTL()).to(testDelayExchange()).with("TestDelayRouting");
    }


    @Bean
    public Queue testDelayReceiveQueue() {
        return new Queue("TestDelayReceiveQueue", true);
    }

    @Bean
    DirectExchange testDelayReceiveExchange() {
        return new DirectExchange("TestDelayReceiveExchange", true, false);
    }
